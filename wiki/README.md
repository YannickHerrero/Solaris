# Solaris Wiki

Welcome to the official wiki for **Solaris**, a terminal-based idle game where you harness the power of the cosmos to generate infinite energy.

## Game Overview

Solaris is an incremental/idle game inspired by classics like Cookie Clicker. Starting with simple Solar Panels, you'll progress through 20 increasingly powerful energy producers, from Mining Drones to Big Bang Catalysts that create pocket universes.

### Quick Stats

| Category | Count |
|----------|-------|
| Producers | 20 |
| Producer Upgrades | 300 |
| Synergy Upgrades | 19 |
| Global Upgrades | 10 |
| Manual Upgrades | 10 |
| **Total Upgrades** | **339** |
| Achievements | 260 |
| Prestige Upgrades | 24 |

---

## Table of Contents

### Getting Started
- [Getting Started Guide](getting-started.md) - First steps, basic mechanics, and early game tips

### Game Content
- [Producers](producers.md) - All 20 energy producers with stats and unlock conditions
- [Upgrades](upgrades.md) - Complete list of 339 upgrades (producer, synergy, global, manual)
- [Achievements](achievements.md) - All 260 achievements across 8 categories
- [Prestige System](prestige.md) - Stellar Ascension, chips, and 24 prestige upgrades

### Reference
- [Controls](controls.md) - Keybindings, UI panels, and layout modes
- [Mechanics](mechanics.md) - Game formulas for costs, production, and bonuses
- [Strategy Guide](strategy.md) - Optimization tips and progression strategies

---

## Key Features

### Energy Production
- **20 Unique Producers**: Each with 5x scaling in cost and production
- **Producer Upgrades**: 15 tiers per producer, each doubling production
- **Synergy System**: Adjacent producers boost each other's output

### Prestige System
- **Stellar Chips**: Earned by ascending, calculated from total energy
- **24 Prestige Upgrades**: Permanent bonuses that persist across runs
- **Multiple Progression Paths**: Production multipliers, starting bonuses, unlock shortcuts

### Quality of Life
- **Offline Progress**: Earn energy while away (up to 8 hours)
- **Auto-Save**: Automatic saving every 30 seconds
- **Boss Mode**: Quick-hide spreadsheet disguise
- **Flexible UI**: Adapts between single-column and two-column layouts

---

## Producer Tiers

| Tier | Producers | Energy Range |
|------|-----------|--------------|
| Early Game | Solar Panel → Orbital Station | 0.1 - 10 E/s |
| Mid Game | Lunar Colony → Star Forge | 50 - 150K E/s |
| Late Game | Neutron Harvester → Dark Matter Engine | 750K - 468M E/s |
| End Game | Cosmic String → Big Bang Catalyst | 2.3B - 1.46T E/s |

---

## Links

- [GitHub Repository](https://github.com/your-username/solaris)
- [Installation Instructions](../README.md)

---

*This wiki documents all game mechanics based on the source code. All values are accurate as of the current version.*
